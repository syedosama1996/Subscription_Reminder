    ...
    try {
      ...
      if (format === 'csv') {
        ...
        if (!useSharing) {
          ...
          try {
            ...
            if (asset) {
              try {
                const album = await MediaLibrary.getAlbumAsync('Subbi Reports');
                if (album) {
                  await MediaLibrary.addAssetsToAlbumAsync([asset], album, false);
                } else {
                  await MediaLibrary.createAlbumAsync('Subbi Reports', asset, false);
                }
              } catch (albumError) {
      ...
      if (savedPath) {
        ...
        if ((format === 'csv' || (format === 'pdf' && Platform.OS === 'android')) && useSharing && await Sharing.isAvailableAsync()) {
          ...
          if (format === 'png') {
            location = 'Photos/Subbi Reports album';
          } else if (format === 'pdf') {
